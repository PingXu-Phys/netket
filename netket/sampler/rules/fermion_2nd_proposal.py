...
class FermionHopRule_with_proposal(ExchangeRule):
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            noise_strength: (ν) Beta distribution concentration parameter.
                Larger values produce less noise. ν→∞ recovers the unperturbed case.
            mixing: (λ) Uniform mixing coefficient in [0, 1). Pulls extreme
                occupations away from 0/1 before Beta sampling, guaranteeing
                numerical stability. 0 means no mixing.
        """
    ...
    def transition(rule, sampler, machine, parameters, state, key, sigma):
        ...
        def _update_samples(key, sigma, hoppable_clusters):
            ...
            # --- per-step Beta perturbation of occupations ---
            # Step 1: uniform mixing floor — pull extreme values to safe region.
            # Step 2: Beta resample — each chain/step gets independent noise.
            # p(u) cancels in MH ratio, so detailed balance holds.
            key, subkey = jax.random.split(key)
            occ_safe = (1.0 - rule.mixing) * rule.occupations + rule.mixing * 0.5
            alpha = rule.noise_strength * occ_safe
            beta_param = rule.noise_strength * (1.0 - occ_safe)
            occ = jax.random.beta(subkey, alpha, beta_param)
            ...
            # Reuse the already-computed forward weight.
            log_w_fwd = _safe_log(weights[cluster], eps)

            # Backward weight: reverse hop swaps source <-> target.
            ...
            # --- incremental Z' ---
            # Only clusters touching site_i or site_j change weight after swap.
            c0 = clusters[:, 0]
            c1 = clusters[:, 1]
            affected = (
                (c0 == site_i) | (c1 == site_i) |
